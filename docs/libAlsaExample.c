...
int main(int argc, char *argv[])
{
    ...
    {
        if ((status = snd_rawmidi_read(device, buffer, 1)) < 0)
        {
            errormessage("Problem reading MIDI input: %s", snd_strerror(status));
        }
        count++;
        if ((unsigned char)buffer[0] >= 0x80)
        { // command byte: print in hex
        ...
        fflush(stdout);
        if (count % 20 == 0)
        { // print a newline to avoid line-wrapping
            printf("\n");
        }
    }

    snd_rawmidi_close(device);
    device = NULL; // snd_rawmidi_close() does not clear invalid pointer,
